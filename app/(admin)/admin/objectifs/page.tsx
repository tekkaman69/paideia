  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {cats.map(cat => (
          <Badge key={cat} variant="secondary" className="bg-primary-50 text-primary-700 border-0 px-3 py-1.5">
        ...
        <CardContent className="p-0">
          {!(goals ?? []).length ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400 mb-4">Créez des objectifs pour les assigner à vos élèves.</p>
              <Link href="/admin/objectifs/new">
                    ...
                    <tr key={goal.id} className="hover:bg-gray-50 group">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          {goal.icon && <span className="text-xl w-8 text-center">{goal.icon}</span>}
                          <div>
                            <p className="font-medium text-gray-800">{goal.title}</p>
                            {goal.description && (
                              <p className="text-xs text-gray-400 mt-0.5 truncate max-w-xs">{goal.description}</p>
                            )}
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        {goal.category ? (
                          <Badge variant="secondary" className="text-xs border-0 bg-primary-50 text-primary-700">{goal.category}</Badge>
                      ...
                      <td className="py-3 px-4">
                        <Badge variant="secondary" className={`text-xs border-0 ${goal.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                          {goal.is_active ? 'Actif' : 'Inactif'}
                        </Badge>
                      </td>
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link href={`/admin/objectifs/${goal.id}`}>
