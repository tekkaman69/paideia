...
export default async function ParentClasseVirtuellePage() {
  ...
  const students = (rawStudents ?? []) as { id: string; display_name: string }[]

  // Fetch upcoming virtual classes for all children
  const studentProfileIds = students.map(s => s.id)

  const { data: upcomingClasses } = studentProfileIds.length
    ? await supabase
        .from('events')
        .select('*, virtual_room:virtual_rooms(*), event_participants!inner(profile_id)')
        .in('event_participants.profile_id', studentProfileIds)
  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1 text-sm">Rejoignez ou suivez les cours en ligne de votre enfant.</p>
      </div>
      ...
      {nextClass ? (
        ...
        <Card className="border-dashed border-2 border-gray-200 shadow-none">
          <CardContent className="p-10 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">La prochaine classe de votre enfant apparaîtra ici.</p>
          </CardContent>
        </Card>
      )}
